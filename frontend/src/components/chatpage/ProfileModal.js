    ...
    <div>
      <Dialog.Root placement="center" bg="white" color="black">
        <Dialog.Trigger asChild>
                {children ? (<span>{children}</span>) : (<Button display={{base:"flex"}}><FontAwesomeIcon icon={faEye}/></Button>)}
        </Dialog.Trigger>
        <Portal>
          <Dialog.Backdrop zIndex="999" />
          <Dialog.Positioner zIndex="1000">
            <Dialog.Content bg="white" color="black " zIndex="1001" height="400px">
              <Dialog.Header>
                <Dialog.Title
  fontSize="40px"
...
>
  {user.name}
              ...
              <Dialog.Body display="flex" flexDir="column" alignItems="center" justifyContent ="space-between">
                <Avatar.Root shape="full" size="2xl" cursor="pointer" boxSize="150px" >
